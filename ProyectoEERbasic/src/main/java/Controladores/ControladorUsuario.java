        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            switch (action) {
                case "/loginUser":
                    ...
                    boolean existeUsuario = daou.login(username, password);
                    session.setAttribute("isLogin", existeUsuario);
                    session.setAttribute("actualUsername", username);
                    response.sendRedirect("/ProyectoEERbasic/Usuario.jsp");
                    break;
                case "/logoutUser":
                ...
                case "/createUser":
                    username = request.getParameter("username");
                    if( daou.getUserByUsername(username) == null){
                        password = request.getParameter("password");
                        name = request.getParameter("name");
                        last_name = request.getParameter("last_name");
                        email = request.getParameter("email");
                        actualUser = new Usuario(username,password,name,last_name,email);
                        regs_afectados = daou.createUser(actualUser);
                    }
                    else {
                        regs_afectados = 2;
                    }
                    session.setAttribute("uCreado", regs_afectados);
                    response.sendRedirect("/ProyectoEERbasic/UsuarioCreado.jsp");
                    break;
                case "/viewUser":
                    username = (String) session.getAttribute("actualUsername");
                    actualUser = daou.getUserByUsername(username);
                    session.setAttribute("actualUser", actualUser);
                    response.sendRedirect("/ProyectoEERbasic/MiCuenta.jsp");
                    break;
                case "/deleteUser":
                    username = (String) session.getAttribute("actualUsername");
                    regs_afectados = daou.deleteUser(username);
                    session.setAttribute("isLogin", false);
                    session.setAttribute("actualUsername", "");
                    response.sendRedirect("/ProyectoEERbasic/Index.jsp");
                    break;
                case "/updateUser":
                    ...
                    email = request.getParameter("email");
                    actualUser = new Usuario(username, password, name, last_name, email);
                    regs_afectados = daou.updateUser(actualUser);
                    session.setAttribute("actualUser", actualUser);
                    response.sendRedirect("/ProyectoEERbasic/MiCuenta.jsp");
                    break;
                default:
